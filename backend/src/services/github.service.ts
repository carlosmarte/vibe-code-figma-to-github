...
import { figmaToReactConverter } from './figma-to-react.service';
...
export class GitHubService {
  ...
  async exportToGitHub(request: GitHubExportRequest): Promise<GitHubPullRequest> {
    ...
    let fileContent = request.componentData;
    
    // Convert to React component if requested
    if (request.convertToReact) {
      try {
        const figmaData = JSON.parse(request.componentData);
        fileContent = figmaToReactConverter.convert(figmaData, request.componentName);
        console.log(`Converted Figma component "${request.componentName}" to React component`);
      } catch (error) {
        console.error('Failed to convert to React component:', error);
        // Fall back to JSON if conversion fails
        request.convertToReact = false;
      }
    }
    
    // Determine the file extension
    const fileExtension = request.fileExtension || (request.convertToReact ? 'tsx' : 'json');
    ...
    if (!filePath.match(/\.[a-zA-Z0-9]+$/)) {
      ...
      const sanitizedName = request.componentName
        ...
        .toLowerCase();
      filePath += `${sanitizedName}.${fileExtension}`;
      console.log(`Adjusted file path from "${request.filePath}" to "${filePath}"`);
    } else if (request.convertToReact) {
      // If converting to React, ensure the file has the correct extension
      filePath = filePath.replace(/\.[^/.]+$/, `.${fileExtension}`);
      console.log(`Updated file extension to .${fileExtension}`);
    }
    
    const commitMessage = `Export Figma component: ${request.componentName}${request.convertToReact ? ' as React component' : ''}`;
...
**File Path:** ${filePath}
**Format:** ${request.convertToReact ? 'React Component (.tsx)' : 'JSON Data'}
**Exported at:** ${new Date().toISOString()}

### Description
This PR contains an exported Figma component${request.convertToReact ? ' converted to a React component' : ' as JSON data'}.
